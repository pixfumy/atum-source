...
public class Atum implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        configFile = new File("config/atum.properties");
    ...
    static void loadFromProperties(Properties properties){
        seed =properties==null||!properties.containsKey("seed")?"": properties.getProperty("seed");
        if(seed==null){
            seed="";
        }
        seed=seed.trim();
        try{
            difficulty =properties==null||!properties.containsKey("difficulty")?1:Integer.parseInt(properties.getProperty("difficulty"));
        } catch (NumberFormatException e) {
            difficulty=1;
        }
        if(difficulty > 4){
